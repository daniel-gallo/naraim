...
def warmup_exponential_decay_cooldown_scheduler(warmup_steps, peak_lr, decay_steps, decay_rate, cooldown_steps, min_lr):
    """
    ...
    """
    warmup_schedule = optax.linear_schedule(init_value=0.0, end_value=peak_lr, transition_steps=warmup_steps)
    ...
    def cooldown_schedule(step):
        cooldown_lr = jnp.maximum(
            min_lr,
            peak_lr * (decay_rate ** (decay_steps / decay_steps)) * (1 - (step / cooldown_steps)),
        )
        return cooldown_lr
    ...
    def schedule_fn(step):
        ...
        decay_lr = jnp.where(decay_step >= 0, exponential_decay_schedule(decay_step), warmup_lr)
